...
RUN apt update && \
        ...
        wget https://github.com/badaix/snapcast/releases/download/v${snapcast_version}/snapclient_${snapcast_version}-1_without-pulse_amd64.deb && \
        apt -f install -y ./snapserver_${snapcast_version}-1_without-pulse_amd64.deb && \
        ...
        apt autoremove -y && \
        apt install -y libasound2 && rm snapserver_${snapcast_version}-1_without-pulse_amd64.deb
